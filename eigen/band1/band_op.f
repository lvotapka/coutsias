...
C    nonsingular and Q_0^1 has a rank 2 component in Null space. Solve
C    in Q_2^M by assuming, in turn, HP = (1,0) or (0,1) + f E Q_2^M
        do j = 0,n
          do k = -q,q
            do i = 0,pn-1
              P(i,k,j) = 0.d0
...
C-- Compute homogeneous solutions in Q_2^M
      ...
      call ul_sol(mmax,m,q,n,P,OP,CP,VP,HP,CHP,HP,cond,0,0,debug,buff1
     _            ,U,bb,cc)
c-- adjust 0-modes
        do j = 0,n
          HP(0,0,j)=1.d0
...
C-- Compute inner products of constraints with homo. solutions
        do j=0, n
          CHP(1,0,j) = CP(0,1,j)*HP(0,0,j) 
          CHP(1,1,j) = CP(0,1,j)*HP(0,1,j)
          CHP(2,0,j) = CP(0,2,j)*HP(0,0,j)
          CHP(2,1,j) = CP(0,2,j)*HP(0,1,j)
          do i = 1, m
            CHP(1,0,j) = CHP(1,0,j)+CP(i,1,j)*HP(i,0,j)
            CHP(1,1,j) = CHP(1,1,j)+CP(i,1,j)*HP(i,1,j)
            CHP(2,0,j) = CHP(2,0,j)+CP(i,2,j)*HP(i,0,j)
            CHP(2,1,j) = CHP(2,1,j)+CP(i,2,j)*HP(i,1,j)
          end do
            if (debug .gt. 2) then
              write(*,*) 'BC matrix: '
              write(*,*) '1,0 = ',CHP(1,0,j)
