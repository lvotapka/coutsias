...
C                 -------<  c o n d 1_GE . f  >-------
C-----------------------------------------------------------------------
		subroutine cond1_ge(maxm,pp,kk,nu,OP,aspect,debug,buff)
C-----------------------------------------------------------------------
C     initialize various banded preconditioners
C     PHASE 1: Gegenbauer family preconditioning
...
c-   0-row
          do j = 0, k
            a2(j) = (nu/(nu+1.d0))*op(1,j-1)+aspect*op(0,j)
          end do
C-  1-row
          do j = -1, k-1
            a1(j) = (2.d0*nu+1.d0)*op(2,j-1)/(2.d0*(nu+2.d0))
     _             +               op(0,j+1)/(2.d0*nu)
     _             +aspect        *op(1,j)
          end do
            a1(k) = (2.d0*nu+1.d0)*op(2,k-1)/(2.d0*(nu+2.d0))
          do j = 0,k
            op(0,j) = a2(j)
          end do
          do j = -1,k
...
C- first compute row entries and store; old contents still needed for next row
          do i = 2, mmax+buff-k
            a1(-k  ) = i         *op(i-1,-k+1)/(2.d0*(i+nu-1.d0))
            do j = -k+1, k-1
              a1(j) =  i         *op(i-1, j+1)/(2.d0*(i+nu-1.d0))
     _               +(i+2.d0*nu)*op(i+1, j-1)/(2.d0*(i+nu+1.d0))
     _               +aspect     *op(i  , j  )
            end do        
            a1(k  ) = (i+2.d0*nu)*op(i+1, k-1)/(2.d0*(i+nu+1.d0))
...
c- zero help arrays
        ...
        i = 1
            a2(-kk-k) =  op(i-1,-kk-k+1)/(2.d0*(i+nu-1.d0))
          do j = -kk-k+1,kk+k-1
            a2( j   ) =  op(i-1, j   +1)/(2.d0*(i+nu-1.d0))
     _                  -op(i+1, j   -1)/(2.d0*(i+nu+1.d0))
          end do
            a2( kk+k) = -op(i+1, kk+k-1)/(2.d0*(i+nu+1.d0))
        do i = 2, mmax+buff-kk-k
            a1(-kk-k) =  op(i-1,-kk-k+1)/(2.d0*(i+nu-1.d0))
          do j = -kk-k+1,kk+k-1
            a1( j   ) =  op(i-1, j   +1)/(2.d0*(i+nu-1.d0))
     _                  -op(i+1, j   -1)/(2.d0*(i+nu+1.d0))
          end do
            a1( kk+k) = -op(i+1, kk+k-1)/(2.d0*(i+nu+1.d0))
          do j = -kk-k,kk+k
